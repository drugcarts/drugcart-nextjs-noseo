...
import { DeleteReviewByService, GetReviewByIdService, GetReviewByService } from '@/services/reviewByService';
    ...
    <Box>
      <Box sx={{ display: "flex" }}>
        ...
        <Button
          ...
          startIcon={<AddIcon />}
          onClick={() => router.push(`/admin/reviewbylist/add`)}
        >
          Add Reviewby
        </Button>
      </Box>
      ...
      <TableContainer component={Paper} sx={{ marginTop: 3 }}>
        <Table size="small" aria-label="simple table">
          ...
          <TableBody>
            {reviewByList && reviewByList?.review_by_lists?.map((row, i) => (
              <TableRow
                key={i}
              ...
              >
                ...
                >
                  <button onClick={() => {
                    router.push(`/admin/reviewbylist/${row?._id}`)
                  }}>
      ...
      >
        <Typography fontFamily={"Poppins"}>Showing 1-{showNo} of {reviewByList?.pagination?.totalItems} entries</Typography>
        <br />
        <Pagination
          size="large"
          count={reviewByList?.pagination?.totalPages}
          page={page}
          color="secondary"
...
export default ReviewByListPage;
