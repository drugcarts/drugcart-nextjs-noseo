...
import { DeleteMetaTagsService, GetMetaTagsIdService, GetMetaTagsListService } from '@/services/metaTagsService';
...
function MetaTagsPage() {
    const { metaTagsList } = useSelector((state) => state.metaTagsData)
...
const handleNoChange = (event) => {
    ...
    console.log("metaTagsList", metaTagsList);
        ...
        <Box>
            <Box sx={{ display: "flex" }}>
                ...
                <Button
                    ...
                    startIcon={<AddIcon />}
                    onClick={() => router.push(`/admin/pagemetalist/add`)}
                >
                    Add Meta List
                </Button>
            </Box>
            ...
            <TableContainer component={Paper} sx={{ marginTop: 3 }}>
                <Table size="small" aria-label="simple table">
                    ...
                    <TableBody>
                        {metaTagsList && metaTagsList?.metaTags_list?.map((row, i) => (
                            <TableRow
                                key={i}
                            ...
                            >
                                ...
                                >
                                    <button onClick={() => {
                                        router.push(`/admin/pagemetalist/${row?._id}`)
                                    }}>
                                ...
                                <DeleteModal
                                    ...
                                    onSubmit={async () => {
                                        await dispatch(DeleteMetaTagsService(row?._id));
                                        setSelectedId(null);
                                    }} />
                            </TableRow>
                        ))}
            ...
            >
                <Typography fontFamily={"Poppins"}>Showing 1-{showNo} of {metaTagsList?.pagination?.totalItems} entries</Typography>
                <br />
                <Pagination
                    size="large"
                    count={metaTagsList?.pagination?.totalPages}
                    page={page}
                    color="secondary"
