...
import { DeleteBlogService, GetBlogIdService, GetBlogService } from '@/services/blogService';
...
function BlogPage() {
    const { blogList, blog } = useSelector((state) => state.blogData)
    ...
    useEffect(() => {
        dispatch(GetBlogService(page, showNo, search))
    }, [page, showNo, search])
    ...
    return (
        <Box>
            ...
            >
                <Typography fontFamily={"Poppins"}>Showing 1-{showNo} of {blogList?.pagination?.totalItems} entries</Typography>
                <br />
                <Pagination
                    size="large"
