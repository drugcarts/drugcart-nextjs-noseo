...
function HealthVideoPage() {
  const { healthVideoList, healthVideo } = useSelector((state) => state.healthVideoData)
  ...
  useEffect(() => {
    dispatch(GetHealthVideosService(page, showNo, search))
  }, [page, showNo, search])
  ...
  console.log("healthVideoList", healthVideoList);
    ...
    <Box>
      <Box sx={{ display: "flex" }}>
        ...
        <Button
          ...
          startIcon={<AddIcon />}
          onClick={() => router.push(`/admin/healthvideo/add`)}
        >
          Add Health Video
        </Button>
      </Box>
      ...
      <TableContainer component={Paper} sx={{ marginTop: 3 }}>
        <Table size="small" aria-label="simple table">
          ...
          <TableBody>
            {healthVideoList && healthVideoList?.health_videos?.map((row, i) => (
              <TableRow
                key={i}
              ...
              >
                ...
                >
                  <button onClick={() => {
                    router.push(`/admin/healthvideo/${row?._id}`)
                  }}>
      ...
      >
        <Typography fontFamily={"Poppins"}>Showing 1-{showNo} of {healthVideoList?.pagination?.totalItems} entries</Typography>
        <br />
        <Pagination
          size="large"
...
export default HealthVideoPage;
