...
import { DeleteQuestionService, GetQuestionIdService, GetQuestionsService } from '@/services/questionService';
...
function UserQuestions() {
    const { questionFormList, question } = useSelector((state) => state.questionData)
    ...
    useEffect(() => {
        dispatch(GetQuestionsService(page, showNo, search))
    }, [page, showNo, search])
        ...
        <Box>
            ...
            <TableContainer component={Paper} sx={{ marginTop: 3 }}>
                <Table size="small" aria-label="simple table">
                    ...
                    <TableBody>
                        {questionFormList && questionFormList?.questions?.map((row, i) => (
                            <TableRow
                                key={i}
                            ...
                            >
                                ...
                                >
                                    <button onClick={() => {
                                        router.push(`/admin/userquestions/${row?._id}`)
                                    }}>
                                        <VisibilityIcon color="primary" />
                                    </button>
                                    <button onClick={async () => {
                                        setOpenModal(true)
                                        await dispatch(GetQuestionIdService(row?._id))
                                    }}>
                                ...
                                <DeleteModal
                                    ...
                                    onSubmit={() => dispatch(DeleteQuestionService(question?._id))} />
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
            ...
            >
                <Typography fontFamily={"Poppins"}>Showing 1-{showNo} of {questionFormList?.pagination?.totalItems} entries</Typography>
                <br />
                <Pagination
                    size="large"
                    count={questionFormList?.pagination?.totalPages}
                    page={page}
                    color="secondary"
