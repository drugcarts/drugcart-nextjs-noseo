...
function HealthNewsPage() {
  const { healthNewsList, healthNews } = useSelector((state) => state.healthNewsData)
  ...
  useEffect(() => {
    dispatch(GetHealthNewsService(page, showNo, search))
  }, [page, showNo, search])
    ...
    <Box>
      <Box sx={{ display: "flex" }}>
        ...
        <Button
          ...
          startIcon={<AddIcon />}
          onClick={() => router.push(`/admin/newslist/add`)}
        >
          Add News
        </Button>
      </Box>
      ...
      <TableContainer component={Paper} sx={{ marginTop: 3 }}>
        <Table size="small" aria-label="simple table">
          ...
          <TableBody>
            {healthNewsList && healthNewsList?.health_news?.map((row, i) => (
              <TableRow
                key={i}
              ...
              >
                ...
                >
                  <button onClick={() => {
                    router.push(`/admin/newslist/${row?._id}`)
                  }}>
                ...
                <DeleteModal
                  ...
                  onSubmit={async () => {
                    await dispatch(DeleteHealthNewsService(row._id));
                    setSelectedId(null);
                  }} />
              </TableRow>
            ))}
      ...
      >
        <Typography fontFamily={"Poppins"}>Showing 1-{showNo} of {healthNewsList?.pagination?.totalItems} entries</Typography>
        <br />
        <Pagination
          size="large"
          count={healthNewsList?.pagination?.totalPages}
          page={page}
          color="secondary"
