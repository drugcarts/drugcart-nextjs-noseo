...
import { DeleteKnowBodyService, GetKnowBodyIdService, GetKnowBodyService } from '@/services/knowBodyService';
...
function KnowBodyPage() {
  ...
  useEffect(() => {
    dispatch(GetKnowBodyService(page, showNo, search))
  }, [page, showNo, search])
  ...
  console.log("knowBodyList", knowBodyList);
    ...
    <Box>
      <Box sx={{ display: "flex" }}>
        ...
        <Button
          ...
          startIcon={<AddIcon />}
          onClick={() => router.push(`/admin/knowbody/add`)}
        >
          Add Know Your Body
        </Button>
      </Box>
      ...
      <TableContainer component={Paper} sx={{ marginTop: 3 }}>
        <Table size="small" aria-label="simple table">
          ...
          <TableBody>
            {knowBodyList && knowBodyList?.know_body_lists?.map((row, i) => (
              <TableRow
                key={i}
              ...
              >
                ...
                >
                  <button onClick={() => {
                    router.push(`/admin/knowbody/${row?._id}`)
                  }}>
      ...
      >
        <Typography fontFamily={"Poppins"}>Showing 1-{showNo} of {knowBodyList?.pagination?.totalItems} entries</Typography>
        <br />
        <Pagination
          size="large"
          count={knowBodyList?.pagination?.totalPages}
          page={page}
          color="secondary"
