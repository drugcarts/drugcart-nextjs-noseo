...
import { DeleteSpecialService, GetSpecialIdService, GetSpecialListService } from '@/services/specialityService';
...
function SpecialListPage() {
    ...
    useEffect(() => {
        dispatch(GetSpecialListService(page, showNo, search))
    }, [page, showNo, search])
        ...
        <Box>
            <Box sx={{ display: "flex" }}>
                ...
                <Button
                    ...
                    startIcon={<AddIcon />}
                    onClick={() => router.push(`/admin/specialtylist/add`)}
                >
                    Add Speciality
                </Button>
            </Box>
                ...
                <Table size="small" aria-label="simple table">
                    <TableHead sx={{ backgroundColor: "#00a65a" }}>
                        <TableRow>
                            ...
                            <TableCell style={rowText}>Speciality</TableCell>
                    ...
                    <TableBody>
                        {specialList && specialList?.specialty_lists?.map((row, i) => (
                            <TableRow
                                key={i}
                            ...
                            >
                                ...
                                >
                                    <button onClick={() => {
                                        router.push(`/admin/specialtylist/${row?._id}`)
                                    }}>
                                ...
                                <DeleteModal
                                    ...
                                    description={`Are you sure you want to delete Speciality ${row?.specialty_name} ?`}
                                    onSubmit={async () => {
                                        await dispatch(DeleteSpecialService(row._id));
                                        setSelectedId(null);
                                    }}
                                />
                            </TableRow>
            ...
            >
                <Typography fontFamily={"Poppins"}>Showing 1-{showNo} of {specialList?.pagination?.totalItems} entries</Typography>
                <br />
                <Pagination
                    size="large"
                    count={specialList?.pagination?.totalPages}
                    page={page}
                    color="secondary"
