...
import { DeleteDoctorService, GetDoctorService } from '@/services/doctorService';
...
function DoctorListPage() {
    ...
    useEffect(() => {
        dispatch(GetDoctorService(page, showNo, search))
    }, [page, showNo, search])
        ...
        <Box>
            <Box sx={{ display: "flex" }}>
                ...
                <Button
                    ...
                    startIcon={<AddIcon />}
                    onClick={() => router.push(`/admin/doctorlist/add`)}
                >
                    Add Doctor
                </Button>
            </Box>
                ...
                <Table size="small" aria-label="simple table">
                    <TableHead sx={{ backgroundColor: "#00a65a" }}>
                        <TableRow>
                            ...
                            <TableCell style={rowText}>Hospital Name</TableCell>
                            <TableCell style={rowText}>Specialist Name</TableCell>
                            <TableCell style={rowText}>Qualification</TableCell>
                    ...
                    <TableBody>
                        {doctorList && doctorList?.doctor_list?.map((row, i) => (
                            <TableRow
                                key={i}
                            ...
                            >
                                ...
                                >
                                    <button onClick={() => {
                                        router.push(`/admin/doctorlist/${row?._id}`)
                                    }}>
                                ...
                                <DeleteModal
                                    ...
                                    description={`Are you sure you want to delete Speciality ${row?.doctor_name} ?`}
                                    onSubmit={async () => {
                                        await dispatch(DeleteDoctorService(row._id));
                                        setSelectedId(null);
                                    }}
                                />
                            </TableRow>
            ...
            >
                <Typography fontFamily={"Poppins"}>Showing 1-{showNo} of {doctorList?.pagination?.totalItems} entries</Typography>
                <br />
                <Pagination
                    size="large"
                    count={doctorList?.pagination?.totalPages}
                    page={page}
                    color="secondary"
